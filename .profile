...
# Hardcoded instead of "$(brew --prefix)": brew resolves through the dogbrew
# shim at 0.5-1s per call (and intermittently errors), and three calls per login
# shell were enough to blow herdr's 2s tab bar command timeout. Homebrew always
# installs to /opt/homebrew on Apple Silicon, so the hardcoded path is safe.
export BASH_COMPLETION_COMPAT_DIR="/opt/homebrew/etc/bash_completion.d"

# Interactive shells only: prompt, aliases, completions, tmux auto-attach.
# Non-interactive login shells (herdr tab-bar commands, cron) skip all of this.
case $- in *i*)
